...
WHERE snapshot.view >= 10000 AND snapshot.view < 100000 GROUP BY snapshot.bvid)
 SELECT song.id, song.name, song.type, video.bvid, video.title, video.pubdate, video.uploader_id, video.song_id, video.copyright, video.thumbnail, video.duration, video.page, snapshot.bvid AS bvid_1, snapshot.date, snapshot.view, snapshot.favorite, snapshot.coin, snapshot."like"
