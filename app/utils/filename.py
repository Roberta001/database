...
def generate_board_file_path(board: str, part: str, issue: int) -> str:
    """
    输入一个榜单，返回文件的相对路径
    """
    board_folder = {
    ...
    part_folder = {"main": "总榜", "new": "新曲榜"}[part]
    if board == "vocaloid-daily":
        ...
        return os.path.join(
            ...
            board_folder,
            part_folder,
            f"{'新曲榜' if part == 'new' else ''}{end_date.strftime('%Y%m%d')}与{start_date.strftime('%Y%m%d')}.xlsx",
        )
        ...
        return os.path.join(
            ...
            board_folder,
            part_folder,
            f"{'新曲' if part == 'new' else ''}{end_date.strftime('%Y-%m-%d')}.xlsx",
        )
        ...
        return os.path.join(
            ...
            board_folder,
            part_folder,
            f"{'新曲' if part == 'new' else ''}{month.strftime('%Y-%m')}.xlsx",
        )
    else:
        raise Exception("输入不符合条件")


def generate_data_file_path(date: datetime) -> str:
    """
    输入一个日期，返回文件的相对路径
    """
