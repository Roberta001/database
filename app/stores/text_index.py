...
from app.utils.text_forms import generate_all_forms, generate_search_variants, normalize_text
...
class TextSearchIndex:
    ...
    async def build_table(self, table_name: str):
        ...
        print(f"[TextIndex] {table_name}: {len(id_to_name)} items, {len(form_to_ids)} forms")
    
    def search(self, table_name: str, keyword: str, limit: int = 200) -> list[TextSearchResult]:
        ...
        for form, items in form_to_ids.items():
            for entity_id, name in items:
                ...
                if form in search_variants:
                    ...
                    ratio = max(len(sv) / len(form) for sv in search_variants if form.startswith(sv) and len(sv) >= 2)
                    score, match_type = 70.0 + ratio * 15, "prefix"
                elif any(sv in form for sv in search_variants if len(sv) >= 2):
                    ratio = max(len(sv) / len(form) for sv in search_variants if sv in form and len(sv) >= 2)
                ...
                if score > 0 and (entity_id not in matches or score > matches[entity_id][0]):
