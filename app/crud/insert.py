...
from ..utils import (
    ...
    read_excel,
    ensure_columns,
    normalize_nullable_int_columns,
    normalize_nullable_str_columns,
)
...
async def resolve_changed_names(
    session: AsyncSession, df: pd.DataFrame, cache: Cache | None = None
):
    if not cache:
        cache = Cache()
    ...
    try:
        ...
        for bvid, new_song_id in video_updates:
            stmt = update(Video).where(Video.bvid == bvid).values(song_id=new_song_id)
...
async def insert_artists(session: AsyncSession, df, cache: Cache | None = None):
    if not cache:
        cache = Cache()
...
async def insert_songs(session: AsyncSession, df, cache: Cache | None = None):
    ...
    name_type_map = {r.name: r.type for r in song_records}
    ...
    new_song_records = [
        SongRecord(name, name_type_map[name]) for name in new_song_names
    ]

    if new_song_records:
        stmt = (
            insert(Song)
            .values([{"name": s.name, "type": s.type} for s in new_song_records])
    ...
    if update_song_records:
        v = (
            values(column("id", Integer), column("type", String))
            .data([(s.id, s.type) for s in update_song_records])
            .alias("v")
        )

        await session.execute(
            update(Song).where(Song.id == v.c.id).values(type=v.c.type)
        )
...
async def insert_relations(
    ...
    new_song_df = df.loc[df["name"].isin(new_song_names)][
        ["name", "synthesizer", "author", "vocal"]
    ].copy()

    # 非严格模式下再检查一遍不需要插入的内容
    if not strict:
        new_song_df = new_song_df.loc[
            new_song_df["synthesizer"].notna()
            & new_song_df["author"].notna()
            & new_song_df["vocal"].notna()
        ].copy()
    ...
    for cls, table, field in (
        ...
        rel_records = set(
            rel_df[["song_id", "artist_id"]].itertuples(index=False, name=None)
        )
        ...
        if new_rel_records:
            new_rel_dicts = [
                {"song_id": t[0], "artist_id": t[1]} for t in new_rel_records
            ]
    ...
    if update:
        ...
        stmt = (
            ...
            .on_conflict_do_update(
                index_elements=["bvid"],
                set_={field: excluded[field] for field in update_cols},
            )
        )
    else:
        stmt = (
...
async def execute_import_snapshots(
    session: AsyncSession, date: str, strict: bool, cache: Cache | None = None
):
    if not cache:
        cache = Cache()
    ...
    # ---------- 原有记录清空 -------------
    delete_stmt = delete(Snapshot).where(Snapshot.date == date_)
            ...
            if not batched_df.empty:
                ...
                stmt = (
                    ...
                    .on_conflict_do_update(
                        ...
                        set_={
                            field: insert(Snapshot).excluded[field]
                            for field in [
                                "view",
                                "favorite",
                                "coin",
...
async def execute_import_rankings(
    ...
    delete_stmt = delete(Ranking).where(
        (Ranking.board == board) & (Ranking.part == part) & (Ranking.issue == issue)
    )
    ...
    try:
        ...
        for i in range(total_batches):
            ...
            if part != "new" and board in ["vocaloid-daily", "vocaloid-weekly"]:
                ...
                insert_df = batch_df.assign(board=board, issue=issue, part=part)[
                    [
                        "board",
                        "part",
                        "issue",
