...
async def init_streak(session: AsyncSession = Depends(get_async_session)):
    all_data = pd.read_excel("收录曲目.xlsx", usecols=['bvid', 'streak'], dtype={'bvid': str, 'streak': int})
    print(len(all_data))
    for start in range(0, len(all_data), 200):
        ...
        v = values(
            column("bvid", String),     # 虚拟值表字段
            column("streak", Integer)
        ).data(
            [(r["bvid"], r["streak"]) for r in rows]
        ).alias("v")

        # SQLAlchemy 的 UPDATE ... FROM 写法
        stmt = (
            update(Video)
            .where(Video.bvid == v.c.bvid)
            .values(streak=v.c.streak)
        )
